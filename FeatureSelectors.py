...
def fsmain(X,y,ls_features,class_option):
    class_names = []
    if(class_option == 0):
    ...
    fig.tight_layout()
    plt.show()
    return(dtc, rfc, gbc, rusboost)


# In[6]:


ls_features1, tot1 = fs(risk_factors_cg,y1,16)
ls_features2, tot2 = fs(risk_factors_cg,y2,16)
...
for el in sort:
    cnt+=1
    if cnt<=16:
        index=np.where(tot==el)
        ls_ind.append(index[0][0])
ls_features=[]
for i in ls_ind:
    ls_features.append(risk_factors_cg.columns[i])
print('these are the',16,'features selected \n', ls_features)


dtc1, rfc1, gbc1, rus1 = fsmain(risk_factors_cg,y1,ls_features,0)
dtc2, rfc2, gbc2, rus2 = fsmain(risk_factors_cg,y2,ls_features,1)

['hsTnl', 'Creatinine', 'Age (Encoded)', 'CRP', 'LDH', 'Urea', 'Ferritin', 'Hb', 'Neutrophil', 'Total bilirubin', 'WBC', 'ALT', 'CK', 'Lymphocyte', 'Co-morbidities', 'Sex (Encoded)']
